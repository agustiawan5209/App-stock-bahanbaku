...
class PesanBahanBakuPacking extends Component
{
    ...
    public $kodepesan, $IDtabOpen, $countTab, $jumlah_stock ,$Alert = false ,$itemID;
    ...
    public function pesanKode()
    {
        $query = pesan::max('id');
        if (empty($query)) {
            $this->kodepesan = 'KP-01';
        } else {
            $exp =  explode("-", $query);
            $str = 'KP';
            $this->kodepesan = sprintf("%s-0%u", $str,  $query + 1);
        }
    ...
    {
        $query = Transaksi::max('id');
        if (empty($query)) {
            $this->kode_transaksi = 'KT-01';
        } else {
            $exp =  explode("T", $query);
            $str = 'KT';
            // $this->kode_transaksi = substr($query,2);
            $this->kode_transaksi = sprintf("%s-0%u", $str,  $query + 1);
        }
    ...
    public function kirim($id)
    {
        ...
        $bahan_stock = BahanBaku::find($id);
        if ($this->jumlah > $this->jumlah_stock) {
            ...
            return redirect()->route('Metode-Pembayaran', ['table' => 'BarangMasukPacking']);
        }
        // BarangMasukAir::create($data);
        // }
    ...
    public function render()
    {
        ...
        $supplier = Suppliers::whereExists(function ($query) {
            $query->select(DB::raw('suppliers_id'))->from('bahan_bakus')->whereColumn('bahan_bakus.suppliers_id', 'suppliers.id');
        })->get();
