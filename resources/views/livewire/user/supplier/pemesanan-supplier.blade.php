<div class="animate__animated animate__fadeIn">
    <div id='recipients' class="p-8 mt-6 lg:mt-0 rounded shadow bg-white">
        ...
        <div class="w-full overflow-hidden">
            <div class="w-full overflow-x-auto">
                ...
                @endif
                @if ($statusModal)
                    @include('components.action.supplier.check-box-supplier')
                @endif
                <div class="w-full overflow-hidden">
                    <div class="w-full overflow-x-auto">
                        <table class="stripe hover w-full whitespace-no-wrap overflow-x-auto"
                            style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
                            <thead
                                class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b   bg-gray-50">
                                <tr class="bg-gray-200">
                                    <th class="font-semibold p-2 text-center text-dark">No</th>
                                    <th class="font-semibold p-2 text-center text-dark">Kode </th>
                                    <th class="font-semibold p-2 text-center text-dark">Bahan Baku</th>
                                    <th class="font-semibold p-2 text-center text-dark">Supplier</th>
                                    <th class="font-semibold p-2 text-center text-dark">Jumlah </th>
                                    <th class="font-semibold p-2 text-center text-dark">Sub Total</th>
                                    ...
                                    <th class="font-semibold p-2 text-center text-dark">Transaksi </th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                @foreach ($barang as $item)
                                    <tr class="text-xs font-medium tracking-wide text-left text-black  border-b   bg-gray-300"
                                        ...
                                        <td class='border border-gray-100 text-xs bg-white text-center'>
                                            {{ $no++ }}</td>
                                        <td class='border border-gray-100 text-xs bg-white text-center text-red-500'>
                                ...
                                @foreach ($barangAir as $item)
                                    <tr>
                                        ...
                                        <td class='border border-gray-100 text-xs bg-white text-center'>
                                            ...
                                            @elseif ($item->status == 'Belum Konfirmasi')
                                                <button type="button"  wire:click='StatusAir({{ $item->id }})'
                                                    class="inline-block px-2 text-xs py-2 mx-auto text-white bg-red-500 rounded-full hover:bg-red-600 md:mx-0">
                                                    {{ $item->status }}
                                                </button>
                                            @endif
                                        ...
                                        <td class='border border-gray-100 text-xs bg-white text-center'>
                                            <ul class="flex justify-around">
                                                <button wire:click='DetailAir({{ $item->id }})'><svg
                                                    class="w-6 h-6 text-blue-500" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                        ...
                        <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                            <nav aria-label="Table navigation">
                                <ul class="inline-flex items-center">
                                    {{ $barang->links('livewire::simple-tailwind') }}
                                </ul>
                            </nav>
                        </span>
                    </div>
    ...
    <script>
        var table = $('#PesananSupplier').DataTable({
