                ...
                <div class="mt-3 text-center">
                    ...
                    <div class="mt-2">
                        ...
                        <div>
                            @can('Manage-Admin')
                                <button type="button" x-on:click="open = false" x-bind:disabled="working"
                                    wire:click='ChangeNot({{ $itemID }})' wire:loading.attr='disabled'
                                    class="inline-block px-5 py-2 mx-auto text-white bg-red-600 rounded-full hover:bg-red-700 md:mx-0">
                                    {{ __('Belum Selesai') }}
                                </button>

                                <button type="button" x-on:click="open = false" x-bind:disabled="working"
                                    wire:click='ChangeYes({{ $itemID }})' wire:loading.attr='disabled'
                                    class="inline-block px-5 py-2 mx-auto text-white bg-blue-600 rounded-full hover:bg-blue-700 md:mx-0">
                                    {{ __('Konfirmasi Pemesanan Barang') }}
                                </button>
                            @endcan
                            @can('Manage-Supplier')
                            <div class="w-full flex justify-center">
                                <img src="{{asset('bukti/'. $image)}}" alt="">
                            </div>
                                <button type="button" x-on:click="open = false" x-bind:disabled="working"
                                    wire:click='ChangeToKonfirmasi({{ $itemID }})' wire:loading.attr='disabled'
                                    class="inline-block px-5 py-2 mx-auto text-white bg-blue-600 rounded-full hover:bg-blue-700 md:mx-0">
                        ...
                        <div class="mt-10 flex justify-center">
                            <span class="mr-2">
                                <button x-on:click="open = false" wire:click='closeModal' x-bind:disabled="working"
