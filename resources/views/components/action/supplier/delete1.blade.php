<div x-data="{ Modal: {{ isset($open) && $open ? 'true' : 'false' }}, working: false }" x-cloak wire:key="delete-{{ $value }}">
    ...
    <div x-show="Modal"
        ...
        <div x-show="Modal" x-transition:enter="ease-out duration-300"
            ...
            class="relative bg-blue-400 rounded-lg px-4 pt-5 pb-4 overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full sm:p-6">
                ...
                <div class="mt-3 text-center">
                    ...
                    <div class="mt-2">
                        <div class="mt-10 text-gray-700">
                            {{ __('Apakah Anda Yakin?') }} {{$value}}
                        </div>
                        <div class="mt-10 flex justify-center">
                            <span class="mr-2">
                                <x-button x-on:click="Modal = false" x-bind:disabled="working"
                            ...
                            <span x-on:click="working = !working"  x-bind:disabled="working">
                                <x-button wire:click="delete1({{ $value }})"
                                    class="w-32 shadow-sm inline-flex justify-center items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-black bg-white hover:bg-white focus:outline-none focus:border-red-700 focus:shadow-outline-teal active:bg-white transition ease-in-out duration-150">
                                    {{ __('Yes') }}
                                </x-button>
                            </span>
                        </div>
